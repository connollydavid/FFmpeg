...
struct SubOCRContext {
    TessBaseAPI *tess;
    int          initialized;
};

av_cold SubOCRContext *sub_ocr_alloc(void)
{
    SubOCRContext *ctx;
...
av_cold void sub_ocr_free(SubOCRContext **pctx)
{
    SubOCRContext *ctx;
...
av_cold int sub_ocr_init(SubOCRContext *ctx, const char *language,
                 const char *datapath)
{
    if (!ctx)
