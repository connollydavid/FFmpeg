...
static int ensure_ocr_context(SubtitleDecContext *ctx, OutputStream *ost)
{
    ...
    ret = sub_ocr_init(ctx->ocr, lang, datapath);
    if (ret < 0) {
                    ...
                    AVSubtitle *sub, AVPacket *pkt)
{
    ...
    for (i = 0; i < sub->num_rects; i++) {
        ...
        if ((!text || strlen(text) < 3) && rect->w > 100) {
            av_free(text);
            text = NULL;
            sub_ocr_pageseg(ctx->ocr, 7);
            ret = sub_ocr_recognize(ctx->ocr,
                                    gray, 1, gw, gw, gh,
                                    &text, &confidence);
            sub_ocr_pageseg(ctx->ocr, 6);
            if (ret < 0) {
                av_free(gray);
                return ret;
            }
