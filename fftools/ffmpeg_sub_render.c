...
av_cold SubRenderContext *sub_render_alloc(int canvas_w, int canvas_h)
{
    SubRenderContext *ctx;
...
av_cold void sub_render_free(SubRenderContext **pctx)
{
    SubRenderContext *ctx;
                     ...
                     int *x, int *y, int *w, int *h)
{
    ...
    ret = sub_render_event(ctx, text, start_ms, duration_ms);
    if (ret < 0)
        return ret;

    return sub_render_sample(ctx, start_ms, rgba, linesize,
                             x, y, w, h, NULL);
}
