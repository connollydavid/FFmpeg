...
int64_t sub_alpha_sum(const uint8_t *rgba, int w, int h,
                      int linesize)
{
    int64_t sum = 0;
    int x, y;
...
enum SubChangeType sub_classify_change(
    ...
    const uint8_t *rgba1, int x1, int y1, int w1, int h1, int ls1)
{
    ...
    if (!has_alpha_diff)
        return position_differs ? SUB_CHANGE_POSITION
                                : SUB_CHANGE_NONE;
...
int sub_find_gap(const uint8_t *rgba, int stride,
                 int w, int h,
...
void sub_scale_alpha(const uint32_t *src, uint32_t *dst,
