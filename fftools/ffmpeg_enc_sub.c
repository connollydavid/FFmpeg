...
static enum AVQuantizeAlgorithm get_quantize_algo(const SubtitleEncContext *ctx)
{
     ...
     * encoder at open, and is never read back from encoder-private state. */
    if (ctx->quantize_method >= AV_QUANTIZE_NEUQUANT &&
        ctx->quantize_method <= AV_QUANTIZE_ELBG)
        return ctx->quantize_method;
...
static int ensure_render_context(SubtitleEncContext *ctx, OutputStream *ost)
{
    ...
    ctx->render = sub_render_alloc(enc_ctx->width, enc_ctx->height);
    if (!ctx->render)
        return AVERROR(ENOMEM);
                                  ...
                                  AVSubtitle *sub)
{
    ...
    /* Constant per encode; NULL when not set. Borrowed from the fftools
     * context (one-way flow; never read back from encoder-private state). */
    ...
    for (i = 0; i < sub->num_rects; i++) {
        ...
        style_forced = ass_style_is_forced(text, ctx->forced_style);

        start_ms    = sub->start_display_time;
        duration_ms = sub->end_display_time - sub->start_display_time;

        ret = sub_render_frame(ctx->render, text,
         ...
         * Palette Definition Segment per Display Set. */
        {
            ...
            if (sub->num_rects < 2 &&
                sub_find_gap(rgba, linesize, rw, rh,
                             &gap_start, &gap_end)) {
                                    ...
                                    const char *text, int events_loaded)
{
    ...
    enum SubChangeType worst_change = SUB_CHANGE_NONE;
    ...
    if (!events_loaded) {
        ret = sub_render_event(ctx->render, text,
                               start_ms, duration_ms);
        if (ret < 0)
            return ret;
    }
    ...
    /* Render first frame */
    ret = sub_render_sample(ctx->render, start_ms,
                            &rgba0, &ls0,
                            &x0, &y0, &w0, &h0, NULL);
    if (ret < 0)
        return ret;
    ...
    for (t = start_ms + frame_ms; t <= start_ms + duration_ms;
        ...
        enum SubChangeType change;

        ret = sub_render_sample(ctx->render, t,
                                &rgba, &ls,
                                &sx, &sy, &sw, &sh, &dc);
        if (ret < 0)
            goto fail;
        ...
        if (!rgba) {
            ...
            int64_t cur_alpha;
            change = sub_classify_change(rgba0, x0, y0, w0, h0, ls0,
                                         rgba, sx, sy, sw, sh, ls);
            cur_alpha = sub_alpha_sum(rgba, sw, sh, ls);
    ...
    if (worst_change == SUB_CHANGE_ALPHA) {
        ...
        ret = sub_render_sample(ctx->render, peak_time,
                                &peak_rgba, &peak_ls,
                                &px, &py, &pw, &ph, NULL);
        if (ret < 0)
            goto fail;
        ...
        /* Epoch Start with first frame's alpha (may be 0% for fade-in) */
        {
            ...
            if (first_rgba && peak_alpha > 0) {
                fa = sub_alpha_sum(first_rgba, fw, fh, fls);
            ...
            sub_scale_alpha(ref_palette, scaled_pal,
                            nb_colors, alpha_pct);
        ...
        for (i = 0; i < n_anim; i++) {
            ...
            ret = sub_render_sample(ctx->render,
                                    anim_times[i],
                                    &rgba, &ls,
                                    &sx, &sy, &sw, &sh,
                                    NULL);
                             ...
                             int64_t pts, AVPacket *pkt)
{
    ...
    for (i = 0; i < ctx->nb_events; i++) {
        ...
        if (first) {
            ret = sub_render_event(ctx->render, ev->text,
                                   ev_start_ms, ev_dur_ms);
            first = 0;
        } else {
            ret = sub_render_add(ctx->render, ev->text,
                                 ev_start_ms, ev_dur_ms);
        }
        if (ret < 0)
            return ret;
    ...
    ret = sub_render_sample(ctx->render, render_ms,
     ...
     * must share one palette (PGS: one PDS per Display Set). */
    {
        ...
        if (nb_active > 1) {
            for (i = 0; i < ctx->nb_events; i++) {
                ...
                ret = sub_render_frame(ctx->render,
        ...
        if (sub_find_gap(rgba, linesize, rw, rh,
                         &gap_start, &gap_end)) {
            int top_h = gap_start;
            int bot_h = rh - gap_end;
                    ...
                    AVSubtitle *sub, AVPacket *pkt)
{
    ...
    if (enc->codec_id == AV_CODEC_ID_HDMV_PGS_SUBTITLE &&
        ...
        if (!text || !text[0])
            return 1; /* nothing to render -- drop event */
