...
#ifndef FFTOOLS_FFMPEG_SUB_RENDER_H
#define FFTOOLS_FFMPEG_SUB_RENDER_H
 ...
 * is not available, sub_render_alloc() returns NULL and all other
 * functions return AVERROR(ENOSYS).
 */

/**
 * Opaque subtitle rendering context.
 */
typedef struct SubRenderContext SubRenderContext;
 ...
 *         not available
 */
SubRenderContext *sub_render_alloc(int canvas_w, int canvas_h);
 ...
 */
void sub_render_free(SubRenderContext **ctx);
 ...
 */
int sub_render_header(SubRenderContext *ctx, const char *header);
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_render_font(SubRenderContext *ctx, const char *name,
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_render_frame(SubRenderContext *ctx, const char *text,
 ...
 * After calling this, use sub_render_sample() to render at one or
 * more timestamps within the event's duration.
 *
 * @param[in] ctx         rendering context
 * @param[in] text        ASS dialogue line
 * @param[in] start_ms    event start time in milliseconds
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_render_event(SubRenderContext *ctx, const char *text,
                     int64_t start_ms, int64_t duration_ms);
 ...
 *
 * Must be called after sub_render_event().
 *
 * @param[in] ctx         rendering context
 * @param[in] text        ASS dialogue line
 * @param[in] start_ms    event start time in milliseconds
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_render_add(SubRenderContext *ctx, const char *text,
                   int64_t start_ms, int64_t duration_ms);

/**
 * Render a snapshot of the current event at a specific time.
 *
 * Must be called after sub_render_event(). Can be called multiple
 * times at different timestamps to sample animation.
 * The caller must free the returned RGBA buffer with av_free().
 *
 * @param[in]  ctx            rendering context
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_render_sample(SubRenderContext *ctx, int64_t render_time_ms,
                      uint8_t **rgba, int *linesize,
                      int *x, int *y, int *w, int *h,
                      int *detect_change);

#endif /* FFTOOLS_FFMPEG_SUB_RENDER_H */
