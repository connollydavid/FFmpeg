...
#ifndef FFTOOLS_FFMPEG_SUB_OCR_H
#define FFTOOLS_FFMPEG_SUB_OCR_H
 ...
 * Tesseract is not available, sub_ocr_alloc() returns NULL and all
 * other functions return AVERROR(ENOSYS).
 */
 ...
 *         not available
 */
SubOCRContext *sub_ocr_alloc(void);
 ...
 */
void sub_ocr_free(SubOCRContext **ctx);
 ...
 */
int sub_ocr_init(SubOCRContext *ctx, const char *language,
                 const char *datapath);

/**
 * Set the Tesseract page segmentation mode.
 *
 * Must be called after sub_ocr_init().  The default is 6
 ...
 * @return 0 on success, negative AVERROR on failure
 */
int sub_ocr_pageseg(SubOCRContext *ctx, int mode);
