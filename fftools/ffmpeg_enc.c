...
/* Build a default ASS subtitle header with the given play resolution,
 * matching the layout produced by libavcodec's ASS helpers.  Local
 * copy: lavc's ff_ass_subtitle_header_full() is not exported from the
 * shared libraries, so fftools cannot call it. */
static int set_ass_subtitle_header(AVCodecContext *enc_ctx,
                                   int play_res_x, int play_res_y)
{
    enc_ctx->subtitle_header = av_asprintf(
        "[Script Info]\n"
        ...
        "&Hffffff,&Hffffff,"    /* {Primary,Secondary}Colour */
        "&H0,&H0,"              /* {Outline,Back}Colour */
    ...
    if (!enc_ctx->subtitle_header)
        return AVERROR(ENOMEM);
    enc_ctx->subtitle_header_size = strlen(enc_ctx->subtitle_header);
    return 0;
}
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        if (dec->subtitle_header) {
            ...
            if (in_desc && out_desc &&
                ...
                ret = set_ass_subtitle_header(enc_ctx,
                                              play_res_x, play_res_y);
                if (ret < 0)
                    return ret;
            }
