...
/* Include the fftools implementation directly (not part of any library) */
...
int main(void)
{
    ...
    /* --- Test 1: Single event bounding box ------------ */
    {
        ...
        sub_render_event(render, text_bottom,
                         0, 3000);
        sub_render_sample(render, 0, &rgba, &ls,
                          &x, &y, &w, &h, NULL);
        if (!rgba) {
    ...
    /* --- Test 2: Composite bounding box (two events) -- */
    {
                         ...
                         0, 3000);
        {
            uint8_t *rgba1 = NULL;
            int ls1, x1, y1, w1, h1;
            sub_render_sample(render, 0, &rgba1, &ls1,
                              &x1, &y1, &w1, &h1, NULL);
            single_y = y1;
            single_h = h1;
            av_free(rgba1);
        ...
        sub_render_event(render, text_bottom,
                         0, 3000);
        sub_render_add(render, text_top,
                       0, 3000);
