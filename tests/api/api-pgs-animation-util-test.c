 ...
 *
 * Tests sub_classify_change(), compute_alpha_ratio(), and
 * sub_scale_alpha() via direct .c inclusion.
 *
...
/* Include the implementation directly (not part of any library) */
#include "fftools/ffmpeg_sub_util.c"
                               ...
                               int w, int h)
{
    ...
    ref_sum = sub_alpha_sum(ref_rgba, w, h, ref_ls);
    cur_sum = sub_alpha_sum(cur_rgba, w, h, cur_ls);
...
static int test_classify_identical(void)
{
    ...
    enum SubChangeType ct;
    int errors = 0;

    fill_rgba(buf, 255, 0, 0, 200);
    ct = sub_classify_change(buf, 10, 20, TEST_W, TEST_H, TEST_LS,
...
static int test_classify_alpha(void)
{
    ...
    ct = sub_classify_change(buf0, 10, 20, TEST_W, TEST_H, TEST_LS,
...
static int test_classify_content(void)
{
    ...
    ct = sub_classify_change(buf0, 10, 20, 4, 4, TEST_LS,
                             buf1, 10, 20, 8, 4, 32);
    if (ct != SUB_CHANGE_CONTENT) {
...
static int test_classify_null(void)
{
    ...
    /* Both NULL -> NONE */
    ct = sub_classify_change(NULL, 0, 0, 0, 0, 0,
                             NULL, 0, 0, 0, 0, 0);
    if (ct != SUB_CHANGE_NONE) {
...
static int test_sub_scale_alpha(void)
{
...
int main(void)
{
    ...
    printf("Testing sub_classify_change...\n");
    ...
    printf("Testing sub_scale_alpha...\n");
    errors += test_sub_scale_alpha();
