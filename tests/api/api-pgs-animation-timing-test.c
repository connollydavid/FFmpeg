 ...
 *
 * Requires libass (for the sub_render API).
 *
...
/* Include the fftools implementations directly (not part of any library) */
#include "fftools/ffmpeg_sub_render.c"
...
int main(void)
{
    ...
    enum SubChangeType worst = SUB_CHANGE_NONE;
    int n_changes = 0;

    /* --- Setup ----------------------------------------- */

    render = sub_render_alloc(1920, 1080);
    if (!render) {
    ...
    alpha_t0   = sub_alpha_sum(rgba0, w0, h0, ls0);
    peak_alpha = alpha_t0;
    ...
    for (int64_t t = start_ms + frame_ms; t <= start_ms + dur_ms;
        ...
        sub_render_sample(render, t, &rgba, &ls,
                          &sx, &sy, &sw, &sh, &dc);

        change = sub_classify_change(
        ...
        if (rgba) {
            alpha = sub_alpha_sum(rgba, sw, sh, ls);
            if (alpha > peak_alpha) {
                peak_alpha = alpha;
                peak_time  = t;
            }
                     ...
                     start_ms, dur_ms);
    {
        ...
        sub_render_sample(render, start_ms,
                          &rgba, &ls,
                          &sx, &sy, &sw, &sh, NULL);
        alpha_t0 = rgba ? sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);

        sub_render_sample(render, 1000,
                          &rgba, &ls,
                          &sx, &sy, &sw, &sh, NULL);
        alpha_mid = rgba ? sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);

        sub_render_sample(render, dur_ms,
                          &rgba, &ls,
                          &sx, &sy, &sw, &sh, NULL);
        alpha_end = rgba ? sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);
    }
    ...
    sub_render_event(render, fade_text,
                     start_ms, dur_ms);
    sub_render_sample(render, peak_time,
                      &peak_rgba, &ls0,
                      &x0, &y0, &w0, &h0, NULL);
    if (!peak_rgba) {
...

    {
        ...
        for (int64_t t = start_ms; t <= start_ms + dur_ms;
            ...
            sub_scale_alpha(ref_pal, scaled, nb_colors, pct);
