                          ...
                          int obj_version)
{
    ...
    /* object_data_length is a 24-bit field */
    if (odl > 0xffffff) {
        av_free(rle);
        return AVERROR(EINVAL);
    }
                         ...
                         int buf_size, const AVSubtitle *h)
{
    ...
    if (h->num_rects > PGS_MAX_OBJECT_REFS) {
        av_log(avctx, AV_LOG_ERROR,
               "Too many subtitle objects (%u, max %d)\n",
               h->num_rects, PGS_MAX_OBJECT_REFS);
        return AVERROR(EINVAL);
    }
    rects = h->num_rects;
    ...
    for (i = 0; i < rects; i++) {
        ...
        if (h->rects[i]->w <= 0 || h->rects[i]->h <= 0) {
            av_log(avctx, AV_LOG_ERROR,
                   "Empty subtitle rectangle\n");
            return AVERROR(EINVAL);
        }
        if (!h->rects[i]->data[0] ||
            h->rects[i]->linesize[0] < h->rects[i]->w) {
            av_log(avctx, AV_LOG_ERROR,
                   "Invalid subtitle bitmap data\n");
            return AVERROR(EINVAL);
        }
        if (!h->rects[i]->data[1]) {
            av_log(avctx, AV_LOG_ERROR,
        ...
        if (h->rects[i]->nb_colors <= 0 ||
            h->rects[i]->nb_colors > AVPALETTE_COUNT) {
            av_log(avctx, AV_LOG_ERROR,
                   "Invalid subtitle palette size %d\n",
                   h->rects[i]->nb_colors);
            return AVERROR(EINVAL);
        }
